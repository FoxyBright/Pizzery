//
// ReviewListRequestFilterInner.swift
//
...
public struct ReviewListRequestFilterInner: Codable, Hashable {
    ...
    /** Параметр, по которому осуществляется фильтрация.   partner_id - фильтрация по партнерам value_type: int64, value_group: single, repeated;   outlet_id - фильтрация по ресторанам value_type: int64, value_group: single, repeated;   rating - фильтрация по оценке value_type: int64, value_group: single, repeated, interval;   created_at - фильтрация по дате создания value_type: int64, value_group: interval;  */
